...
// 공통데이터 더미 (colorClass를 computed나 함수로 처리하기 위해 가공)
const events = ref([
  ...
  { id: 3, title: '한화 호텔앤리조트 콜라보 이벤트 캠페인', start: '2026-04-27', projectManager:'한화 호텔앤리조트', end: '2026-05-02'},
...
<template>
  <div class="w-full min-h-screen flex flex-col transition-colors duration-300"
    ...
    <div class="px-6 py-4 border-b flex items-center justify-between sticky top-0 z-10 transition-colors duration-300"
         :class="isDark ? 'bg-[#1e1e2d] border-[#2d2d3f]' : 'bg-white border-gray-100'">
      
      <h2 class="text-xl font-bold transition-colors" :class="isDark ? 'text-white' : 'text-gray-900'">
        일정 한눈에 보기
      </h2>
      ...
      <div class="flex items-center">
        <template v-for="(view, index) in viewOptions" :key="view.id">
          ...
          <div 
            ...
            class="w-[1px] h-3 mx-1 transition-colors"
            :class="isDark ? 'bg-[#2d2d3f]' : 'bg-gray-300'"
          ></div>
        </template>
      </div>
    ...
    <main class="flex-1 overflow-auto">
      <transition name="fade" mode="out-in">
        <div :key="currentView" class="h-full">
          <MainCalendar v-if="currentView === 'calendar'" :eventsData="formattedEvents" />
          <MainTable v-else-if="currentView === 'table'" :eventsData="formattedEvents" />
          <MainTimeline v-else-if="currentView === 'timeline'" :eventsData="formattedEvents"/>
        </div>
      </transition>
    </main>
  </div>
