...
import { getPartnerOrganizationCandidates, invitePartnerGroup } from '@/api/campaignMembers'
...
const filtered = computed(() => {
  ...
  return candidates.value.filter((c) =>
    ...
    || (c.representativeGm?.name || '').toLowerCase().includes(q)
    || (c.representativeGm?.email || '').toLowerCase().includes(q)
  )
})
...
onMounted(async () => {
  loading.value = true
  try {
    const res = await getPartnerOrganizationCandidates(props.campaignId)
...
async function invite(c) {
  submittingOrg.value = c.organizationIdx
  error.value = ''
  try {
    await invitePartnerGroup(props.campaignId, c.organizationIdx)
    window.alert('그룹 초대를 발송했습니다.')
    emit('invited')
    emit('close')
  } catch (e) {
    const status = e?.response?.status
    if (status === 409) error.value = '이미 발송된 초대가 있습니다.'
    else if (status === 403) error.value = '권한이 없습니다.'
    else if (status === 400) error.value = e?.response?.data?.message ?? '초대를 보낼 수 없습니다.'
    else error.value = '초대 중 오류가 발생했습니다.'
  } finally {
    submittingOrg.value = null
  }
}
</script>
    ...
    <div class="cm-modal-overlay" role="presentation" @click.self="emit('close')">
      <section class="modal-shell" role="dialog" aria-modal="true">
        <div class="modal-header">
          <div>
            <div class="modal-header__eyebrow"><span>CAMPAIGN · 그룹 초대</span></div>
            <h2>협력사 그룹 초대</h2>
          </div>
        ...
        <div class="modal-body">
          <input v-model="search" class="fld" placeholder="조직명·대표 GM 이름·이메일로 검색" />

          <div v-if="loading" class="cm-modal-empty">로딩 중...</div>
          <div v-else-if="filtered.length === 0" class="cm-modal-empty">초대 가능한 협력사가 없습니다.</div>
          <ul v-else class="cm-modal-list">
            <li v-for="c in filtered" :key="c.organizationIdx">
              <span class="cm-modal-item">
                <span class="cm-org">
                  <span class="cm-badge" aria-hidden="true">{{ initials(c.organizationName) }}</span>
                  <strong>{{ c.organizationName }}</strong>
                </span>
                <small>합류 가능 인원 {{ c.eligibleCount }}명 · 대표 GM {{ c.representativeGm?.name }} ({{ c.representativeGm?.email }})</small>
              </span>
              <button type="button" class="btn btn--primary cm-invite-btn"
                      :disabled="submittingOrg === c.organizationIdx"
                      @click="invite(c)">
                초대
              </button>
            </li>
...
.cm-modal-item { display: flex; flex-direction: column; gap: 2px; }
.cm-org { display: inline-flex; align-items: center; gap: 8px; }
.cm-badge { display: inline-grid; place-items: center; width: 24px; height: 24px; border-radius: 8px; font-size: 11px; font-weight: 700; background: var(--panel-muted); color: var(--text-secondary); }
